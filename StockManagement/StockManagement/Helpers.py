...
def getExludeFields():
    return ['id', 'billno', 'created_at','updated_at','domain_user_id','added_by_user_id','created_by_user_id','updated_by_user_id','is_staff','is_superuser','is_active','plan_type','last_login','last_device','date_joined','last_ip','domain_name']
    ...
    for field in model_instance._meta.fields:
        ...
        if checkisFileField(field.name):
            ...
            if isinstance(field, ForeignKey):
                ...
                # Vérifier si la clé primaire est `billno` ou `id`
                related_pk = 'billno' if 'billno' in [f.name for f in related_model._meta.fields] else related_model._meta.pk.name

                # Construire la liste des options pour le champ select
                if hasattr(related_model, 'defaultkey'):
                    related_key_name = related_model.defaultkey()
                    options = related_model.objects.filter(domain_user_id=domain_user_id).values_list(related_pk, related_key_name, related_model.defaultkey())
                else:
                    related_key_name = related_model._meta.pk.name
                    options = related_model.objects.filter(domain_user_id=domain_user_id).values_list(related_pk, related_key_name, 'name')

                fielddata['options'] = [{'id': option[0], 'value': option[1]} for option in options]
                fielddata['type'] = 'select'
                
                # Sélectionner la valeur par défaut en fonction de la clé primaire utilisée
                model_dict = model_to_dict(model_instance)
                fielddata['default'] = model_dict.get(field.name, '')
